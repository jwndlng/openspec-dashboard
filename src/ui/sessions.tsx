...
const noop = async () => {};
const Context = createContext<SessionUi>({ config: null, snapshot: null, sessions: [], agents: [], worktrees: [], shown: [], hidePane: () => {}, focusPane: () => {}, focusTick: { tick: 0 }, requestEnd: () => {}, openPanel: () => {}, start: noop, refresh: noop });
...
export function SessionProvider({ config, snapshot = null, children }: { config: Config | null; snapshot?: Snapshot | null; children: ComponentChildren }) {
  ...
  const value = useMemo(() => ({ config, snapshot, sessions, agents, worktrees, shown, hidePane, focusPane, panelId, endingId, focusTick, requestEnd, error, openPanel, start, refresh }), [config, snapshot, sessions, agents, worktrees, shown, hidePane, focusPane, panelId, endingId, focusTick, error, openPanel, start, refresh]);
