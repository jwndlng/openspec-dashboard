...
- The navigation **moves with the content**: on wide screens it starts at the top-left next to the first section and is moved along beside the section currently in view — level with that section's start, following into view while a section taller than the window is read, and gliding to the next section when that one becomes current. It is not pinned to the top of the page. (Intermediate versions brought it along on a jump, made it sticky, and then let it scroll out of view entirely; in use neither the sticky nor the vanishing navigation was what was wanted.)
- Jumping works the same (section to the top, focus, reduced motion), and the next jump is always one click away.
- The **narrow-screen row** sits above the sections and scrolls away with them; moving it along would cover the sections. The `<nav>` itself is the row's sideways scroller, which also fixes entries of a sideways-scrolled row not reacting to clicks in Chrome.
...
- `src/ui/settingsNav.tsx`: view tracking measures against the new scroll container; the "keep current entry visible" effect scrolls the row horizontally only; the wide navigation is placed beside the current section.
- `test/`: pure helpers for the horizontal row offset and the navigation's placement with unit tests; the existing section tests keep passing.
