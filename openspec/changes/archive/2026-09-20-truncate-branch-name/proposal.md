...
- Truncation keeps the **end** of the branch name visible by shortening the middle (`⎇ feat/intro…quota-enforcement` rather than `⎇ feat/introduce-resource-c…`): the tail is what distinguishes branches of similar changes, and the type prefix (`feat/`, `fix/`, `chore/`) stays readable too. The `⎇` glyph always stays visible.
...
- `kanban-board`: **Cards show repository, name, progress, activity and branch** gains the rule that the branch badge stays within the card, shortening long names in the middle with an ellipsis (start and end stay readable) while exposing the full name via tooltip and accessible name; scenarios for a long and a short branch name are added.
...
- `src/ui/styles.css`: a truncating badge variant (`max-width: 100%`, a shrinking head part with `text-overflow: ellipsis` and a non-shrinking tail part) and `min-width: 0` on the card meta row so flex children may shrink; token-based, no new colours.
- `src/ui/format.ts` + `test/format.test.ts`: a pure helper that splits a branch name into head and tail for the badge.
