...
- [x] 1.1 Land the two spec deltas in `specs/` as written — `change-creation` (the staging requirement, the amended
...
- [x] 1.2 Update invariant 1 in `CLAUDE.md`: the enumerated exception list goes from four entries to five, naming
...
- [x] 2.1 Add a private `git()` runner to `src/server/createChange.ts` (D2): `Bun.spawn(["git", ...args])` with `cwd`
...
- [x] 2.2 Call it once after both file writes succeed, as `git add -- openspec/changes/<name>/` (D1, D3), and widen
...
- [x] 2.3 Make the failure paths return `staged: false` without failing the create (D4, D5): non-git repository,
...
- [x] 2.4 Verify the scoping and the non-escalation in `test/createChange.test.ts`: in a temp git repository that
...
- [x] 3.1 Return `{ name: result.name, staged: result.staged }` from the create route in `src/server/api.ts`, and
...
- [x] 3.2 Extend `test/createChangeApi.test.ts` with a git-backed harness repository (reuse the `git` /
...
- [x] 3.3 Verify every refusal still runs no git at all: extend the existing `treeFingerprint` refusal assertions in
