...
The repository board SHALL show a header with a breadcrumb linking back to the overview, the repository name, its path in monospace, an action that copies `cd <path>` (shell-quoted) to the clipboard, the current branch when known, the number of worktrees, the relative age of the repository's last update, and any repository warnings or scan error. The dashboard MUST NOT execute the copied command.
...
- **THEN** the header shows `Projects / alpha-infra`, the path, `main`, `3 worktrees` and `updated 1d ago`
