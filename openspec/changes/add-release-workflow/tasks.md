...
- [ ] 1.1 Add `src/server/version.ts` exporting `VERSION`: the `--define`d global `OPENSPEC_DASHBOARD_BUILD_VERSION`
...
- [ ] 1.2 Handle `--version` in `parseArgs` in `src/server/index.ts` next to `--help`: print `VERSION`, exit 0, before
...
- [ ] 1.3 Add `scripts/build.ts` running the current compile command (`bun build --compile src/server/index.ts --outfile
...
- [ ] 1.4 Add `./dist/openspec-dashboard --version` to the smoke-test step in `.github/workflows/ci.yml`. Verify by
      reading the workflow and by CI passing on the pull request.

## 2. Labels and the draft release

- [ ] 2.1 Add `.github/release-drafter.yml`: `name-template`/`tag-template` `v$RESOLVED_VERSION`, the source's
...
- [ ] 2.2 Add a `label` job to `.github/workflows/pr-title.yml` running `release-drafter/release-drafter/autolabeler`
      ...
      write-scope scenario and, after merge, that this change's own PR got the `feat` label.
- [ ] 2.3 Add `.github/workflows/draft-release.yml`: on push to `main`, workflow permissions `contents: read`, one job
...
- [ ] 3.1 Add `.github/workflows/release.yml`, trigger `release: types: [published]`, workflow permissions
...
- [ ] 3.2 Add the `publish` job (`needs: build`, permissions `contents: write`, `id-token: write`,
...
- [ ] 3.3 Pin every new action to a full commit SHA with its version in a comment, matching the existing workflows.
      Verify with `grep -n 'uses:' .github/workflows/*.yml` showing only 40-character SHAs.

## 4. Documentation and first release

- [ ] 4.1 Add a Releasing section to `CONTRIBUTING.md` (labels from titles, draft on every merge, version rules, `major`
