...
- Shared/UI: `src/shared/types.ts` (artifact file types), `src/ui/routes.ts` (new route + path helper), new `src/ui/changeDetail.tsx` and `src/ui/markdown.tsx`, `src/ui/api.ts`, `src/ui/app.tsx` (route switch), `src/ui/kanban.tsx` (card link), `src/ui/styles.css` (detail layout and Markdown typography, both themes).
...
- Tests: route parsing, endpoint validation (traversal, unknown repo/change, oversize file, archived change), Markdown sanitisation cases, using the synthetic fixtures.
