...
- [x] 2.2 Render it as a `badge warning` in the detail header (`ChangeFacts`, `src/ui/changeDetail.tsx`) next to the pending-archive badge; verify in `test/changeDetail.test.ts` that the header of a leftover shows it and an ordinary archive's does not, and by running `bun run dev` against a temp repo with a leftover and seeing a single `Archived` card, no card in `Synced`, and the badge in its detail view
