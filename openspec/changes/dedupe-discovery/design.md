...
`git.ts` gains `originUrl(cwd)` (`git config --get remote.origin.url`, read-only). `discoverRepos` looks up the remote for every candidate and every configured repo (bounded concurrency 8, failures → no remote), normalises it (`git@host:org/repo(.git)` and `https://host/org/repo(.git)` → `host/org/repo`, lower-cased host), and sets `sameRemoteAs: { name, path, tracked }[]` on candidates that share a remote with any other known repository, tracked or candidate. Candidates are included because on a fresh setup nothing is tracked yet — exactly when the hint is most useful. The remote is only looked up for a directory that is itself a git top level (it has its own `.git`): inside a bigger repository `git config` answers with the enclosing repository's remote, which would flag every sibling project of a monorepo — and this project's own test fixtures — as clones of each other. The response type becomes `DiscoveredRepo = RepoConfig & { sameRemoteAs?: … }`; the UI strips the extra field when moving a candidate into the draft config.
...
The `dashboard-api` delta was rebased on 2026-09-21 onto the requirement text as it stood after `shared-openspec-config` and `run-agent-actions-from-ui` were archived: "The dashboard never writes to tracked repositories" keeps its enumerated exceptions, and this change only adds `config --get` to the read-only git list plus one scenario. Rebased again while implementing (same day), after the work-status change landed: the block now carries the current text (`diff` in the git list, work statuses, the "Reading work statuses has no side effects" scenario). The `Config endpoints` and `Discover endpoint` blocks were re-checked against the main spec: they still extend it without dropping anything.
