...
test("every outcome reads as text, with the reason available", () => {
  expect(pullOutcome(result({ update: "fast-forwarded", commits: 3 }))).toMatchObject({ label: "+3 commits", tone: "ok" });
  ...
  expect(pullOutcome(result({ update: "skipped", reason: "on feat/redesign, not main; only fetched" }))).toEqual({ label: "fetched only", tone: "warn", detail: "Fetched, but the checkout was not updated: on feat/redesign, not main; only fetched." });
